...
def scale_coords(img1_shape, coords, img0_shape, ratio_pad=None):
    """
    This function fixes the interpolation of the load_images function
    which stretches both height and width so their ratios are 1:1
    The height is further padded to be dividable by 64 so hence we
    subtract padding and divides by width gain
    """
...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, 
    multi_label=False, labels=(), max_det=300, num_rot=6):
    ...
    nc = prediction.shape[2] - 5 - num_rot # number of classes
    ...
    # output = [torch.zeros((1, 6+num_rot), device=prediction.device)] * prediction.shape[0]
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        x[:, 5:-num_rot] *= x[:, 4:5]  # conf = obj_conf * cls_conf
        ...
        if multi_label:
            ...
            i, j = (x[:, 5:-num_rot] > conf_thres).nonzero(as_tuple=False).T
            # x = torch.cat((box[i], x[i, j + 5, None], j[:, None].float()), 1)
            x = torch.cat((box[i], x[i, j + 5, None], j[:, None].float(), x[i, -num_rot:]), 1)
        else:  # best class only
            # conf, j = x[:, 5:].max(1, keepdim=True)
            conf, j = x[:, 5:-num_rot].max(1, keepdim=True)
            # x = torch.cat((box, conf, j.float()), 1)[conf.view(-1) > conf_thres]
            x = torch.cat((box, conf, j.float(), x[:, -num_rot:]), 1)[conf.view(-1) > conf_thres]
