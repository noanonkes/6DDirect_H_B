...
from visualize.plotting import convert_euler_bbox_to_6dof, plot_3axis_Zaxis
...
def get_image(img, euler, x1, y1, x2, y2):
    imz = img.copy()
    ...
    global_pose = convert_euler_bbox_to_6dof(euler, [x1, y1, x2, y2], global_intrinsics)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--imgsz", type=int, nargs="+", default=[720, 1280, 3])
    parser.add_argument("--weights", default="weights/agora_m_1280_e300_t40_lw010/best.pt")
    ...
    pd_rot_data = output_dict["pd_rot_data"][mask]
    gt_rot_data = output_dict["gt_rot_data"][mask]
    ...
    for i, (img, error, pd_euler, gt_euler, pd_6D, gt_6D, pd_bbox, gt_bbox) in enumerate(pbar):
        ...
        _x2 = min(w, int(x1-EXTRA)+int(x2+EXTRA*2))
        _y2 = min(h, int(y1-EXTRA)+int(y2+EXTRA*2))
        box_w = _x2 - _x1
        box_h = _y2 - _y1

        # Empty image for original, ground truth, prediction, and text
        concat_img = np.zeros((2 * box_h, 2 * box_w, c))
        ...
        img_pd = get_image(im0, pd_euler, x1, y1, x2, y2)
        ...
        img_gt = get_image(im0, gt_euler, x1, y1, x2, y2)
