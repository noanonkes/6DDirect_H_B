...
def mean_absolute_error_calculate(gt_json_path, pd_json_path, frontal_face):
    matched_iou_threshold = 0.65  # our nms_iou_thre is 0.65
    score_threshold = 0.7
...
def mean_absolute_error_calculate_v2(gt_json_path, pd_json_path, frontal_face):
    ...
    for gt_label_dict in tqdm(gt_labels_list):  # matching for each GT label
        ...
        [gt_pitch, gt_yaw, gt_roll] = gt_label_dict['euler_angles']
         ...
         # this is not a head
        if gt_label_dict["category_id"] != 1:
            false_status += 1
            continue
    ...
    total_num = len(gt_labels_list) - false_status
    left_num = len(gt_data)
    
    if left_num == 0:
        return total_num, [30,60,30], 2, [30,60,30], 1, [30,60,30], 1, torch.tensor(0.)
    ...
    gt_zyx = torch.tensor(gt_data)
    gt_zyx = torch.stack((gt_zyx[:, 2], gt_zyx[:, 1], gt_zyx[:, 0])).T
    gt_data_rot = euler_angles_to_matrix(torch.deg2rad(gt_zyx), "ZYX")

    pd_zyx = torch.tensor(pd_data)
    pd_zyx = torch.stack((pd_zyx[:, 2], pd_zyx[:, 1], pd_zyx[:, 0])).T
    pd_data_rot = euler_angles_to_matrix(torch.deg2rad(pd_zyx), "ZYX")

    geo_loss = GeodesicLoss()(pd_data_rot, gt_data_rot)

    return total_num, pose_matrix, left_num, pose_matrix_b, left_num_b, pose_matrix_f, left_num_f, geo_loss
