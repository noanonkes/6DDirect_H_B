...
def get_smpl_vertices(
    kid_flag, gt, gender, smpl_neutral_kid, smpl_neutral, pose2rot=True
):
    ...
    return (
        smplx_gt.joints.detach().cpu().numpy().squeeze(),
...
def get_smplx_vertices(
    ...
    if kid_flag:
        ...
        if gender == "female":
            ...
            raise KeyError("Kid: Got gender {}, what gender is it?".format(gender))
    ...
    smplx_gt = model_gt(
        ...
        reye_pose=torch.tensor(gt["reye_pose"], dtype=torch.float),
        pose2rot=pose2rot,
    )
...
def get_pred_joints(predJoints, predVerts, df, idx, pNum, args):
    ...
    if "allSmplJoints3d" in df.iloc[idx].at["pred"][pNum].keys():
        predJoints.append(df.iloc[idx].at["pred"][pNum]["allSmplJoints3d"].squeeze())
        ...
        if args.modeltype == "SMPLX":
            pred_joints, pred_verts = get_smplx_vertices(
                num_betas,
                kid_flag,
                gt,
                gender,
...
def get_projected_joints(
    ...
    if args.modeltype == "SMPL":
        smpl_path = os.path.join(
            args.gt_model_path, df.iloc[idx].at["gt_path_smpl"][pNum]
        ).replace(".obj", ".pkl")
        ...
        smplx_path = os.path.join(
            args.gt_model_path, df.iloc[idx].at["gt_path_smplx"][pNum]
        ).replace(".obj", ".pkl")
        gt = pickle.load(open(smplx_path, "rb"))
        gt_joints_local, gt_verts_local = get_smplx_vertices(
            args.numBetas,
            kid_flag,
            gt,
            gender,
    ...
    gt_verts_cam_2d, gt_verts_cam_3d, cam_v3d, _, _, _ = project_2d(
        args, df, idx, pNum, gt_verts_local
    )
    gt_joints_cam_2d, gt_joints_cam_3d, cam_j3d, camR, camT, camK = project_2d(
        args, df, idx, pNum, gt_joints_local
    )
    # print("Output: gt_verts_cam_2d, gt_verts_cam_3d, gt_joints_cam_2d, gt_joints_cam_3d, cam_j3d, cam_v3d, camR, camT, camK,")
    return (
        gt_verts_cam_2d,
        gt_verts_cam_3d,
        gt_joints_cam_2d,
...
def add_joints_verts_in_dataframe(args, df, store_joints):
    ...
    df.insert(column="gt_verts_2d", loc=len(df.columns), value=len(df) * [""])
    df.insert(
        column="cam_j3d", loc=len(df.columns), value=len(df) * [""]
    )  # newly added for Euler angles Calculation
    df.insert(
        column="camR", loc=len(df.columns), value=len(df) * [""]
    )  # newly added for Euler angles Calculation
    df.insert(
        column="camT", loc=len(df.columns), value=len(df) * [""]
    )  # newly added for Euler angles Calculation
    df.insert(
        column="camK", loc=len(df.columns), value=len(df) * [""]
    )  # newly added for Euler angles Calculation
    ...
    for idx in tqdm(range(len(df))):
        for jdx in range(len(df.iloc[idx]["isValid"])):
            if jdx == 0:
                if store_joints:
                    df.iloc[idx, df.columns.get_loc("gt_verts_2d")] = []
            ...
            if store_joints:
                ...
                # df.iloc[idx].at["gt_joints_2d"].append(gt_joints_cam_2d) # pixel
                df.iloc[idx].at["gt_verts_2d"].append(gt_verts_cam_2d) # pixel
                # df.iloc[idx].at["gt_joints_3d"].append(gt_joints_cam_3d) # camera
                # df.iloc[idx].at["gt_verts_3d"].append(gt_verts_cam_3d) # camera
                df.iloc[idx].at["cam_j3d"].append(cam_j3d) # world
                # df.iloc[idx].at["cam_v3d"].append(cam_v3d) # world
